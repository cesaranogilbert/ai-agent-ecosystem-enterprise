    ...
    def _get_reporting_structure(self) -> Dict[str, List]:
        ...
        for agent_id, agent in self.hierarchy.items():
            structure[agent.name] = {
                "reports_to": self.hierarchy.get(agent.reporting_to).name if agent.reporting_to and self.hierarchy.get(agent.reporting_to) else "None",
                "direct_reports": [
                    self.hierarchy.get(report_id).name 
                    for report_id in agent.direct_reports 
                    if report_id in self.hierarchy and self.hierarchy.get(report_id)
                ]
            }
